...
export default function registerRoutes(app, deps) {
    ...
    function validate(req, res, next) {
        ...
        if (!errors.isEmpty()) return sendJSONError(res, 400, 'Validierungsfehler. Bitte überprüfe deine Eingabe.', errors.array());
        next();
    }

    function requireAdmin(req, res, next) {
        if (!req.user?.isAdmin) return sendJSONError(res, 403, 'Forbidden');
        next();
    }
...
`
    ...
    // Routen
    // Essentielle Routen für Authentifizierung, Sicherheit und Logging

    app.post('/api/app-gate/login',
        dashboardLimiter,
    ...
    // Dieser Endpoint erfordert keine Middleware, damit Nutzer die Email auch verifizieren koennen, wenn sie den Link in einem anderen Browser (bspw. dem Standard-Browser, der vom Email-Anbieter geöffnet wird.) öffnen können.
...
Hinweis: Es handelt sich bei der Authentifizierung nicht um eine klassische mit Benutzerkonten o. Ä., sondern um eine äußere Authentifizierung einer Seite. Die Website ist also nur für bestimmte autorisierte Personen, die von den Administratoren das allgemeine Passwort erhalten haben. Es gibt also nicht mehrere Accounts, sondern nur ein Passwort, das eingegeben werden muss, um durch die Authentifizierung zu kommen. Die Attempt Hashes sind dabei hashes der versuchten Passwörter. Und beachte: Sei weder zu lasch, noch sieh jede abfolge vonf alcshen  passwörtern udn fehlschlägen als angriff. Entscheide intelligent und hilfreich.
    ...
    app.post('/api/admin/timetable/subs',
        requireAppGate(appGateSecret),
        requireUser(userSecret, BannedUser, User),
        requireAdmin,
        validateCsrf(csrfSecret),
    ...
    app.delete('/api/admin/timetable/subs/:id',
        requireAppGate(appGateSecret),
        requireUser(userSecret, BannedUser, User),
        requireAdmin,
        validateCsrf(csrfSecret),
    ...
    app.delete('/api/admin/subjects/:name',
        requireAppGate(appGateSecret),
        requireUser(userSecret, BannedUser, User),
        requireAdmin,
        validateCsrf(csrfSecret),
    ...
    app.delete('/api/admin/users/:id/ban',
        requireAppGate(appGateSecret),
        requireUser(userSecret, BannedUser, User),
        requireAdmin,
        validateCsrf(csrfSecret),
