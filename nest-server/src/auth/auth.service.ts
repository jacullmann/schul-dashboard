...
export class AuthService {
  ...
  private generateMfaPendingToken(res: Response, userId: string, email: string) {
  ...
  async login(email: string, password: string, res: Response, ip?: string) {
    ...
    const { data: user } = await sb
      .from('users')
      .select('id, email, password_hash, email_verified, mfa_enabled, mfa_secret, user_roles(roles(name), tenant_id)')
    ...
    const { data: ban } = await sb.from('banned_users').select('id').eq('user_id', user.id).maybeSingle();
    if (ban) {
    ...
    this.setAuthToken(res, user.id as string, user.email as string, globalRole, firstGroup?.tenant_id || null);

    return { ok: true, csrfToken: newCsrfToken };
  }

  async verifyMfa(code: string, userId: string, email: string, res: Response, ip?: string) {
    const sb = this.supabaseService.getClient();
    const { data: user } = await sb
      .from('users')
      .select('id, email, mfa_enabled, mfa_secret, user_roles(roles(name), tenant_id)')
      .eq('id', userId)
      .maybeSingle();
    ...
    const secret = await decryptData(user.mfa_secret as any, userId);
    const isValid = authenticator.check(code, secret);

    if (!isValid) {
      await sb.from('user_activity').insert({ user_id: userId, type: 'auth:mfa_login_failed', meta: { ip } });
      await new Promise((r) => setTimeout(r, 100 + Math.random() * 100));
      throw new UnauthorizedException('Authentifizierung fehlgeschlagen');
    }
    ...
    await sb.from('user_activity').insert({ user_id: userId, type: 'auth:mfa_login', meta: {} });
  ...
  async register(email: string, password: string) {
    if (isWeakPassword(password)) {
      throw new BadRequestException('Passwort muss mindestens 8 Zeichen lang sein und Buchstaben sowie Zahlen enthalten.');
    }

    const sb = this.supabaseService.getClient();
    const { data: exists } = await sb.from('users').select('id').eq('email', email.toLowerCase()).maybeSingle();
    if (exists) {
    ...
    const { data: user, error } = await sb
      .from('users')
      .insert({ email: email.toLowerCase(), password_hash: passwordHash, email_verified: false })
      .select()
      .single();
    ...
    await sb.from('verifications').insert({ email: user.email, token, expires_at: expiresAt });

    const clientVerifyUrl = this.configService.get<string>('CLIENT_VERIFY_URL') || `http://localhost:5173/verify`;
    const verifyUrl = `${clientVerifyUrl}?token=${token}`;

    try {
      await this.emailService.sendVerificationEmail(user.email, verifyUrl);
      return { ok: true, message: 'Registriert. Bitte überprüfe deine E-Mail sowie deinen Spamordner.' };
    } catch (mailErr) {
      return { ok: true, message: 'Registriert. E-Mail konnte nicht versendet werden. Bitte später erneut oder Support kontaktieren.' };
    }
  }
  ...
  async getMe(userId: string, activeGroupId: string | null) {
    ...
    const tenantRoleName = userRoles?.find((ur) => ur.tenant_id === activeGroupId)?.roles?.name || null;
  ...
  async deleteMe(userId: string, res: Response) {
    const sb = this.supabaseService.getClient();
    const { data: user } = await sb.from('users').select('id, user_roles(roles(name))').eq('id', userId).maybeSingle();
  ...
  async verifyEmail(token: string) {
    const sb = this.supabaseService.getClient();
    const { data: ver } = await sb.from('verifications').select('*').eq('token', token).maybeSingle();
    if (!ver) throw new BadRequestException('Ungültiger Token');
    if (dayjs(ver.expires_at).isBefore(dayjs())) throw new BadRequestException('Token abgelaufen');

    const { data: user } = await sb.from('users').select('id').eq('email', ver.email).maybeSingle();
    if (!user) throw new BadRequestException('Nutzer nicht gefunden');
  ...
  async forgotPassword(email: string) {
    ...
    if (!user) {
      return { ok: true, message: 'Wenn die E-Mail existiert, wurde ein Code versendet.' };
    }
    ...
    await sb.from('password_resets').insert({ email, code, expires_at: expiresAt });
  ...
  async verifyResetToken(email: string, code: string) {
    ...
    if (dayjs(pr.expires_at).isBefore(dayjs())) throw new BadRequestException('Code abgelaufen');
  ...
  async resetPassword(resetToken: string, password: string) {
    ...
    const sb = this.supabaseService.getClient();
    const { data: user } = await sb.from('users').select('id, mfa_enabled').eq('email', email).maybeSingle();
    ...
    await sb.from('user_activity').insert({
      ...
      meta: { by: 'self', mfaWasEnabled: !!user.mfa_enabled, mfaDisabled: true },
    });
    ...
    return { ok: true, message: 'Passwort wurde geändert. MFA wurde deaktiviert.' };
  }
  ...
  async changePassword(userId: string, currentPassword: string, newPassword: string) {
    ...
    const sb = this.supabaseService.getClient();
    const { data: user } = await sb.from('users').select('id, password_hash').eq('id', userId).maybeSingle();
    if (!user) throw new BadRequestException('Nutzer nicht gefunden');

    const isCorrect = await bcrypt.compare(currentPassword, user.password_hash as string);
    if (!isCorrect) throw new ForbiddenException('Aktuelles Passwort ist falsch');

    const newPasswordHash = await bcrypt.hash(newPassword, 12);
    await sb.from('users').update({ password_hash: newPasswordHash }).eq('id', userId);
    await sb.from('user_activity').insert({ user_id: userId, type: 'account:password_change', meta: { by: 'self' } });
