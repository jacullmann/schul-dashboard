...
export class EmailService {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    if (resendApiKey) {
      ...
      this.resendClient.domains
        ...
        .catch((err) =>
          this.logger.error('Resend API Test fehlgeschlagen:', err instanceof Error ? err.stack : String(err)),
        );
    } else {
  ...
  async sendVerificationEmail(to: string, verifyUrl: string): Promise<unknown> {
    ...
    if (data && 'id' in data) {
      this.logger.log(`Verification email sent successfully. Resend ID: ${data.id}`);
    }
    return data;
  }
