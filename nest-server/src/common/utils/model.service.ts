...
export class ModelService {
  ...
  buildThumbUrl(secureUrl: string): string {
    try {
  ...
  generateCloudinaryUrl(
    ...
    const cloudinaryCloudName = this.configService.get('CLOUDINARY_CLOUD_NAME') || 'dwysdpvcm';
  ...
  withThumb(img: any): any {
    if (img.publicId && (!img.url || img.metadata)) {
      const meta = img.metadata;
      return {
        url: this.generateCloudinaryUrl(
          img.publicId,
          meta?.version,
          meta?.format as string | undefined,
          false,
        ),
        thumbUrl: this.generateCloudinaryUrl(
          img.publicId,
          meta?.version,
          meta?.format as string | undefined,
          true,
    ...
    return {
      url: img.url || '',
      thumbUrl: img.thumbUrl || this.buildThumbUrl(img.url || ''),
      publicId: img.publicId,
      createdBy: img.createdBy,
    };
  }

  timeLeftColor(dueDate: string | Date): string {
    const now = dayjs();
