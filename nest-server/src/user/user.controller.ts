...
import { UpdatePersonalizationDto, UpdateSetupDto, VisibilityStatusDto } from './dto/user.dto';
...
export class UserController {
  ...
  updatePersonalization(@CurrentUserId() userId: string, @Body() body: UpdatePersonalizationDto) {
  ...
  setVisibility(
    ...
    return this.userService.setVisibility(tenantId, itemId, userId, body.status);
  }

  @Delete('items/:id/visibility')
  removeVisibility(@Param('id') itemId: string, @CurrentUserId() userId: string) {
    return this.userService.removeVisibility(itemId, userId);
  }

  @Post('activity/pageload')
  logPageLoad(@CurrentUserId() userId: string, @Headers('user-agent') userAgent: string) {
