...
export class UserService {
  ...
  async updateSetup(userId: string, globalRole: string, enrKurs?: string, wpuKurs1?: string, wpuKurs2?: string, theater: number = 0) {
    const sb = this.supabaseService.getClient();
  ...
  async getChecks(userId: string) {
    const sb = this.supabaseService.getClient();
    const { data } = await sb.from('keep_checked').select('item_id').eq('user_id', userId);
    return { itemIds: (data || []).map((d) => d.item_id) };
  }

  async getPins(userId: string) {
    const sb = this.supabaseService.getClient();
    const { data } = await sb.from('pinned_items').select('item_id').eq('user_id', userId);
    return { itemIds: (data || []).map((d) => d.item_id) };
  }

  async getVisibility(userId: string) {
    const sb = this.supabaseService.getClient();
    const { data } = await sb.from('user_item_visibility').select('item_id, status').eq('user_id', userId);
  ...
  async setVisibility(tenantId: string, itemId: string, userId: string, status: string) {
    const sb = this.supabaseService.getClient();
    const { data: item } = await sb.from('items').select('id').eq('id', itemId).eq('tenant_id', tenantId).maybeSingle();
  ...
  async checkItem(tenantId: string, itemId: string, userId: string) {
    ...
    await sb.from('keep_checked').upsert(
      { item_id: itemId, user_id: userId, checked_at: new Date().toISOString() },
      { onConflict: 'item_id,user_id' }
    );

    await sb.from('user_activity').insert({ user_id: userId, type: 'item:check', meta: { itemId } });
    return { ok: true };
  }
  ...
  async uncheckItem(itemId: string, userId: string) {
    ...
    await sb.from('user_activity').insert({ user_id: userId, type: 'item:uncheck', meta: { itemId } });
    return { ok: true };
  }
  ...
  async pinItem(tenantId: string, itemId: string, userId: string) {
    ...
    await sb.from('pinned_items').upsert(
      { item_id: itemId, user_id: userId, pinned_at: new Date().toISOString() },
      { onConflict: 'item_id,user_id' }
    );

    await sb.from('user_activity').insert({ user_id: userId, type: 'item:pin', meta: { itemId } });
    return { ok: true };
  }
  ...
  async unpinItem(itemId: string, userId: string) {
    ...
    await sb.from('user_activity').insert({ user_id: userId, type: 'item:unpin', meta: { itemId } });
    return { ok: true };
  }
}
