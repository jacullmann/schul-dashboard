...
export class ItemsService {
  ...
  async getItems(tenantId: string, userId: string, type: string, filter?: string) {
    ...
    let q = sb.from('items').select('*, creator:users!items_created_by_fkey(email)').eq('tenant_id', tenantId);
  ...
  async getItemById(tenantId: string, id: string) {
    const sb = this.supabaseService.getClient();
    const { data: row } = await sb.from('items').select('*').eq('id', id).eq('tenant_id', tenantId).maybeSingle();
    
    if (!row) throw new NotFoundException('Eintrag nicht gefunden');

    const { data: creator } = await sb.from('users').select('email').eq('id', row.created_by).maybeSingle();
  ...
  async createItem(tenantId: string, userId: string, payload: any) {
    ...
    for (const img of rawImages) {
      if (!img.publicId) throw new BadRequestException('Fehlendes publicId im Bild');
    }
    ...
    if (error || !item) throw new InternalServerErrorException('Fehler beim Erstellen des Eintrags');

    await sb.from('user_activity').insert({ user_id: userId, type: 'item:create', meta: { id: item.id, type: item.type } });
  ...
  async updateItem(tenantId: string, id: string, userId: string, payload: any) {
    const sb = this.supabaseService.getClient();
    const { data: item } = await sb.from('items').select('id, created_by').eq('id', id).eq('tenant_id', tenantId).maybeSingle();
    
    if (!item) throw new NotFoundException('Nicht gefunden');
    if (item.created_by !== userId) throw new ForbiddenException('Nur der Ersteller kann diesen Eintrag bearbeiten.');

    if (payload.dueDate) {
      const due = dayjs(payload.dueDate);
      if (due.isBefore(dayjs().subtract(2, 'day'))) throw new BadRequestException('Datum liegt zu weit in der Vergangenheit');
      if (due.isAfter(dayjs().add(365, 'day'))) throw new BadRequestException('Datum liegt zu weit in der Zukunft');
    }

    if (payload.images) {
      if (payload.images.length > 12) throw new BadRequestException('Max 12 Bilder');
      const userCount = payload.images.filter((i: any) => i.createdBy === userId || !i.createdBy).length;
    ...
    await sb.from('items').update(update).eq('id', item.id).eq('tenant_id', tenantId);
    await sb.from('user_activity').insert({ user_id: userId, type: 'item:update', meta: { id: item.id } });

    return { ok: true };
  }

  async deleteItem(tenantId: string, id: string, userId: string, globalRole: string, tenantRole: string | undefined) {
    const sb = this.supabaseService.getClient();
    const { data: item } = await sb.from('items').select('id, created_by, images').eq('id', id).eq('tenant_id', tenantId).maybeSingle();
    ...
    await sb.from('user_activity').insert({ user_id: userId, type: 'item:delete', meta: { id: item.id } });

    return { ok: true };
  }

  async updateItemNote(tenantId: string, id: string, userId: string, editorNote: string) {
    const sb = this.supabaseService.getClient();
    const { data: item } = await sb.from('items').select('id').eq('id', id).eq('tenant_id', tenantId).maybeSingle();
    
    if (!item) throw new NotFoundException('Nicht gefunden');

    const noteContent = editorNote.trim();
    await sb.from('items').update({ editor_note: noteContent }).eq('id', item.id).eq('tenant_id', tenantId);
    await sb.from('user_activity').insert({ user_id: userId, type: 'item:note:update', meta: { itemId: item.id } });
  ...
  async addImage(tenantId: string, id: string, userId: string, imgBody: any) {
    const sb = this.supabaseService.getClient();
    const { data: item } = await sb.from('items').select('id, images').eq('id', id).eq('tenant_id', tenantId).maybeSingle();
    ...
    await sb.from('items').update({ images }).eq('id', item.id).eq('tenant_id', tenantId);
    await sb.from('user_activity').insert({ user_id: userId, type: 'item:image:add', meta: { itemId: item.id } });

    return { ok: true, image: withThumb(newImage) };
  }

  async deleteImage(tenantId: string, itemId: string, publicIdRaw: string, userId: string, globalRole: string, tenantRole: string | undefined) {
    const sb = this.supabaseService.getClient();
    const { data: item } = await sb.from('items').select('id, images, created_by').eq('id', itemId).eq('tenant_id', tenantId).maybeSingle();
    ...
    images.splice(idx, 1);
    await sb.from('items').update({ images }).eq('id', item.id).eq('tenant_id', tenantId);
    await sb.from('user_activity').insert({ user_id: userId, type: 'item:image:delete', meta: { itemId: item.id } });
  ...
  async reportItem(userId: string, email: string, payload: any) {
    ...
    await sb.from('user_activity').insert({ user_id: userId, type: 'item:report', meta: { itemId, category } });
  ...
  async createUploadSignature() {
    ...
    const folder = this.configService.get<string>('CLOUDINARY_FOLDER') || 'hausaufgaben';
