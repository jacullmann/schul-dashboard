...
export class GroupAdminService {
  ...
  async getStats(tenantId: string) {
    ...
    const { data: subsData } = await sb.from('timetable_subs').select('id').eq('tenant_id', tenantId);
    ...
    const { count: memberCount } = await sb.from('user_roles').select('*', { count: 'exact', head: true }).eq('tenant_id', tenantId);
  ...
  async changeMemberRole(tenantId: string, currentUserId: string, targetUserId: string, newRole: string) {
    if (targetUserId === currentUserId) throw new BadRequestException('Du kannst deine eigene Rolle nicht ändern.');
    ...
    const sb = this.supabaseService.getClient();
    const { data: existing } = await sb.from('user_roles').select('id, role_id').eq('user_id', targetUserId).eq('tenant_id', tenantId).maybeSingle();
    
    if (!existing) throw new NotFoundException('Nutzer ist kein Mitglied dieser Gruppe');

    await sb.from('user_roles').update({ role_id: roleId }).eq('id', existing.id);
    await sb.from('user_activity').insert({ user_id: currentUserId, type: 'group-admin:change-role', meta: { targetUserId, newRole, tenantId } });

    return { ok: true, message: `Rolle zu "${newRole}" geändert.` };
  }

  async removeMember(tenantId: string, currentUserId: string, targetUserId: string) {
    if (targetUserId === currentUserId) throw new BadRequestException('Du kannst dich nicht selbst entfernen.');

    const sb = this.supabaseService.getClient();
    const { data: targetRole } = await sb.from('user_roles').select('id, roles(name)').eq('user_id', targetUserId).eq('tenant_id', tenantId).maybeSingle();

    if (!targetRole) throw new NotFoundException('Nutzer ist kein Mitglied dieser Gruppe');
    
    if ((targetRole as any).roles?.name === 'admin') {
      throw new ForbiddenException('Admins können nicht entfernt werden.');
    }

    await sb.from('user_roles').delete().eq('user_id', targetUserId).eq('tenant_id', tenantId);
    await sb.from('user_activity').insert({ user_id: currentUserId, type: 'group-admin:remove-member', meta: { targetUserId, tenantId } });
  ...
  async renameGroup(tenantId: string, userId: string, name?: string) {
    ...
    const sb = this.supabaseService.getClient();
    const { data: existing } = await sb.from('groups').select('id').eq('name', name).maybeSingle();
    ...
    await sb.from('user_activity').insert({ user_id: userId, type: 'group-admin:rename-group', meta: { tenantId, newName: name.trim() } });
  ...
  async cleanupOldItems(tenantId: string, userId: string) {
    ...
    await sb.from('user_activity').insert({ user_id: userId, type: 'group-admin:cleanup', meta: { deletedCount: itemIds.length } });
  ...
  async getTimetableSubs(tenantId: string) {
    const sb = this.supabaseService.getClient();
    const { data: subs } = await sb.from('timetable_subs').select('*').eq('tenant_id', tenantId);
    return (subs || []).map((s) => ({
      id: s.id,
  ...
  async createTimetableSub(tenantId: string, userId: string, sub: any) {
    ...
    await sb.from('user_activity').insert({ user_id: userId, type: 'timetable:sub:create', meta: { lessonId: sub.lessonId } });
  ...
  // --- Announcements ---
  async createAnnouncement(tenantId: string, userId: string, content: string, color?: string, showAsPopup?: boolean) {
    const sb = this.supabaseService.getClient();
  ...
  async deleteAnnouncement(tenantId: string, id: string) {
    const sb = this.supabaseService.getClient();
    const { data: ann } = await sb.from('announcements').select('id').eq('id', id).eq('tenant_id', tenantId).maybeSingle();
    
    if (!ann) throw new NotFoundException('Nicht gefunden');

    await sb.from('announcements').delete().eq('id', id).eq('tenant_id', tenantId);
    return { ok: true };
  }
}
