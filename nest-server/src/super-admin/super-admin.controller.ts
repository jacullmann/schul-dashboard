...
import { UpdateUserRoleDto, ProcessReportDto, CreateSubjectDto, AdminCreateTimetableSubDto } from './dto/super-admin.dto';
...
export class SuperAdminController {
  ...
  cleanupOldItems(@ActiveTenantId() tenantId: string, @CurrentUserId() currentUserId: string) {
  ...
  @Post('users/:id/ban')
  banUser(@Param('id') targetUserId: string, @CurrentUserId() adminUserId: string) {
    return this.superAdminService.banUser(targetUserId, adminUserId);
  }

  @Delete('users/:id/ban')
  unbanUser(@Param('id') targetUserId: string, @CurrentUserId() adminUserId: string) {
  ...
  @Patch('users/:id')
  updateUserRole(@Param('id') targetUserId: string, @Body() body: UpdateUserRoleDto) {
    return this.superAdminService.updateUserRole(targetUserId, body.role);
  }

  @Delete('users/:id/activity/prune')
  pruneActivity(@Param('id') targetUserId: string, @CurrentUserId() adminUserId: string) {
  ...
  processReport(
    ...
    return this.superAdminService.processReport(reportId, adminUserId, body.processed);
  }

  @Delete('reports/:id')
  deleteReport(@Param('id') reportId: string, @CurrentUserId() adminUserId: string) {
    return this.superAdminService.deleteReport(reportId, adminUserId);
  }

  @UseGuards(TenantGuard)
  @Post('subjects')
  upsertSubject(@ActiveTenantId() tenantId: string, @Body() body: CreateSubjectDto) {
    return this.superAdminService.upsertSubject(tenantId, body.name);
  }

  @UseGuards(TenantGuard)
  @Delete('subjects/:name')
  deleteSubject(@ActiveTenantId() tenantId: string, @Param('name') name: string) {
  ...
  deleteTimetableSub(@ActiveTenantId() tenantId: string, @Param('id') id: string) {
