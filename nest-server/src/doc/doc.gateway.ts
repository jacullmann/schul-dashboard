...
export class DocGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  private connectedAdmins = new Map<string, { email: string; userId: string }>();
  ...
  async handleConnection(client: Socket): Promise<void> {
    try {
      ...
      const { data: ban } = await sb.from('banned_users').select('id').eq('user_id', payload.sub).maybeSingle();
      if (ban) {
        client.disconnect();
        return;
      }
      ...
      this.connectedAdmins.set(client.id, { email: payload.email, userId: payload.sub });

      await this.docService.loadFromDb();
      this.startPersistTimer();

      const docState = this.docService.getDocState();
      client.emit('doc:init', { ...docState, onlineAdmins: this.getOnlineList() });
      client.broadcast.emit('doc:admins-update', { onlineAdmins: this.getOnlineList() });
    } catch {
      client.disconnect();
    }
  }
  ...
  async handleDisconnect(client: Socket): Promise<void> {
    ...
    this.server?.emit('doc:admins-update', { onlineAdmins: this.getOnlineList() });
  ...
  handleDocUpdate(
    ...
    const result = this.docService.applyUpdate(payload.content, payload.clientVersion, email);
