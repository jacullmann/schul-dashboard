...
export class MfaService {
  ...
  async getStatus(userId: string) {
    ...
    const { data: user } = await sb.from('users').select('mfa_enabled').eq('id', userId).maybeSingle();
  ...
  async setup(userId: string) {
    const sb = this.supabaseService.getClient();
    const { data: user } = await sb.from('users').select('email, mfa_enabled').eq('id', userId).maybeSingle();

    if (!user) throw new NotFoundException('Nutzer nicht gefunden');
    if (user.mfa_enabled) throw new BadRequestException('MFA ist bereits aktiviert');
    ...
    await sb.from('user_activity').insert({ user_id: userId, type: 'mfa:setup:started', meta: {} });
  ...
  async activate(userId: string, code: string) {
    ...
    const encryptedSecret = await encryptData(secret, userId);
    await sb.from('users').update({ mfa_enabled: true, mfa_secret: encryptedSecret }).eq('id', userId);

    // Delete pending secret
    await sb.from('mfa_pending_secrets').delete().eq('user_id', userId);

    await sb.from('user_activity').insert({ user_id: userId, type: 'mfa:activated', meta: {} });
  ...
  async deactivate(userId: string, code: string, ip?: string) {
    ...
    // Disable MFA
    await sb.from('users').update({ mfa_enabled: false, mfa_secret: null }).eq('id', userId);

    await sb.from('user_activity').insert({ user_id: userId, type: 'mfa:deactivated', meta: { ip } });
