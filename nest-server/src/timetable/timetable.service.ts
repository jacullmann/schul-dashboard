...
export class TimetableService {
    ...
    try {
      ...
      if (userId) {
        const { data: user } = await sb
          .from('users')
          .select('personalized, done_setup, enr_kurs, wpu_kurs_1, wpu_kurs_2, theater')
          .eq('id', userId)
          .maybeSingle();
      ...
      return filteredLessons.map((l: any) => ({
        ...
        subjects: l.subjects ? { id: l.subjects.id, name: l.subjects.name } : null,
        courses: l.courses ? { id: l.courses.id, name: l.courses.name } : null,
      }));
    } catch (err) {
      throw new InternalServerErrorException('Fehler beim Laden des Stundenplans');
    }
  }
  ...
  async getSubs(tenantId: string) {
    ...
    try {
      ...
      throw new InternalServerErrorException('Fehler beim Laden der Substitutions');
    }
  }
  ...
  async getSubjects(tenantId: string) {
    const sb = this.supabaseService.getClient();
    try {
      const { data } = await sb.from('subjects').select('id, name, short').eq('tenant_id', tenantId).order('name');
      return data || [];
    } catch {
  ...
  async getPersons(tenantId: string) {
    const sb = this.supabaseService.getClient();
    try {
      const { data } = await sb.from('persons').select('id, name, short, title, person_subjects').eq('tenant_id', tenantId).order('name');
      return (data || []).map((p) => ({
        id: p.id,
        name: p.name,
        short: p.short,
  ...
  async getDaltonSchedule(tenantId: string) {
    const sb = this.supabaseService.getClient();
    try {
      const { data } = await sb.from('dalton_schedule').select('*').eq('tenant_id', tenantId);
      return (data || []).map((r) => ({
        id: r.id,
        room: r.room,
        size: r.size,
      ...
      throw new InternalServerErrorException('Fehler beim Laden der Dalton-Pläne');
    }
  }
  ...
  async getAnnouncements(tenantId: string) {
    const sb = this.supabaseService.getClient();
    try {
      const { data } = await sb.from('announcements').select('*').eq('tenant_id', tenantId).order('created_at', { ascending: false }).limit(5);
      return (data || []).map((a) => ({
        id: a.id,
        content: a.content,
        color: a.color,
      ...
      throw new InternalServerErrorException('Fehler beim Laden der Ankündigungen');
    }
  }
}
