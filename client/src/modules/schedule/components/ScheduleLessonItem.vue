...
<template>
  ...
  >
    ...
    <div v-else>
      ...
      >
        <template v-if="lesson._original && getDisplayName(lesson) !== getDisplayName(lesson._original)">
          <span class="line-through text-on-surface-muted mr-1 font-normal group-[.highlight-active]:text-surface-hover-border" :class="isSelected && 'text-surface-hover-border'">
            {{ getDisplayName(lesson._original) }}
          </span>
          <span class="font-bold text-on-surface group-[.highlight-active]:text-on-action" :class="isSelected && 'text-on-action'">
            {{ getDisplayName(lesson) }}
          </span>
        </template>
        <template v-else>
      ...
      >
        <span class="inline-flex gap-1 items-center">
          <template v-if="lesson._original && lesson.room !== lesson._original.room">
            <span class="line-through text-on-surface-muted mr-1 font-normal group-[.highlight-active]:text-surface-hover-border" :class="isSelected && 'text-surface-hover-border'">
