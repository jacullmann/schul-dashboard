...
import type {
  ...
  ViewState,
  SortOption,
} from '@/modules/infodashboard/types';
...
export function useArticleSearch() {
  ...
  const searchResults = computed(() => {
    if (!activeSearchQuery.value.trim() && currentView.value === 'search')
      return articles;
    if (!activeSearchQuery.value.trim()) return [];

    const queryTerms = activeSearchQuery.value
      .toLowerCase()
      .split(/\s+/)
      .filter((q) => q.length > 0);
    ...
    const scored = articles.map((article) => {
      ...
      const contentCleaned =
        stripHtml(contentText) + ' ' + stripHtml(excerptText);
      const contentLower = contentCleaned.toLowerCase();

      const wordsCleaned = (titleLower + ' ' + contentLower).replace(
        /[.,/#!$%^&*;:{}=\-_`~()"]/g,
        '',
      );
  ...
  const processedArticles = computed(() => {
    let list =
      currentView.value === 'search' || activeSearchQuery.value
        ? searchResults.value
        : articles;
