...
const AVATAR_COLORS = [
    '#AA47BD', '#7B1FA2', '#77919D', '#455A65',
    '#EC417A', '#C1175C', '#0388D2', '#0098A7',
    '#004D40', '#EF6C00', '#F6511E',
];
...
export function useAccountMenu(
    ...
    const avatarLetter = computed(() => {
        return props.email && props.email.length > 0 && props.email[0] ? props.email[0].toUpperCase() : '?';
    });
    ...
    function handleLogout() { emit('logout'); close(); }
    function openSetup() { emit('openSetup'); close(); }
    function openChangePassword() { showChangePassword.value = true; close(); }
    function openSecurity() { showSecurity.value = true; close(); }
    function startDelete() { showDeleteAccount.value = true; close(); }

    function onMfaChanged(enabled: boolean) { emit('mfaChanged', enabled); }
    function onAccountDeleted() { emit('deleted'); }
    function onDeleteError(msg: string) { emit('error', msg); }
