...
export function useSubjectAdmin() {
    ...
    async function loadSubjects() {
        loading.value = true;
        try {
            const { data } = await hw.get<AdminSubject[]>('/api/group-admin/subjects');
            subjects.value = data || [];
        } catch {
    ...
    async function createSubject(name: string) {
        ...
        saving.value = true;
        try {
            const { data } = await hw.post<AdminSubject>('/api/group-admin/subjects', {
                name: name.trim(),
            });
    ...
    async function updateSubject(id: string, updates: { name?: string; isActive?: boolean }) {
        saving.value = true;
        try {
    ...
    async function deleteSubject(id: string) {
        if (!confirm('Fach wirklich löschen? Falls es noch referenziert wird, schlage stattdessen eine Deaktivierung vor.')) return;
        try {
