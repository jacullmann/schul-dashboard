  ...
  <div class="test-wrapper number-memory-test">
    <div v-if="state !== 'result'" class="full-height flex-center">
      <div v-if="state === 'idle'" class="center-content text-center">
        ...
        <p class="test-sub">
          Merk dir die Zahl, die gleich auf dem Bildschirm erscheint.
        </p>
      ...
      <div
        v-if="state === 'showing'"
        class="center-content showing-content text-center"
      >
        <div class="number-display">{{ currentNumber }}</div>
        <div class="progress-bar-container">
          <div
            class="progress-bar"
            :style="{ animationDuration: showingTime + 'ms' }"
          ></div>
        </div>
      </div>
      ...
      <div
        v-if="state === 'input'"
        class="center-content input-content text-center"
      >
        ...
        <form @submit.prevent="checkAnswer" class="input-form">
          <input
            ...
            autofocus
          />
          <BaseButton type="submit" class="test-btn primary w-full mt-3"
            >Prüfen</BaseButton
          >
        </form>
      </div>
      ...
      <div
        v-if="state === 'feedback'"
        class="center-content feedback-content text-center"
      >
        <div v-if="isCorrect">
          <h2 class="correct-text test-heading-sm">Richtig!</h2>
          <p class="test-sub">Die Zahl war {{ currentNumber }}.</p>
          <BaseButton class="test-btn primary mt-4" @click="nextLevel()"
            >Nächstes Level</BaseButton
          >
        </div>
        <div v-else>
          <h2 class="wrong-text test-heading-sm">Falsch</h2>
          <p class="test-sub">
            Die richtige Zahl war <strong>{{ currentNumber }}</strong
            >.
          </p>
          <p class="test-sub">
            Deine Eingabe war
            <strong class="wrong-color">{{ userInput }}</strong
            >.
          </p>
          <BaseButton class="test-btn primary mt-4" @click="endGame()"
            >Weiter</BaseButton
          >
        </div>
      </div>
    ...
    <div
      v-if="state === 'result'"
      class="center-content result-content text-center full-height flex-center"
    >
      <div class="icon-wrap"><Brain :size="64" /></div>
      <h2 class="test-heading">Level {{ level - 1 }} erreicht!</h2>
      <p class="test-sub">
        Du konntest dir eine {{ level - 1 }}-stellige Zahl merken.
      </p>
