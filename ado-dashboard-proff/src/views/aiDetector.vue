...
<template>
  <div class="full">
    ...
    <footer>
      ...
      1.0 PRIMÄRE ZWECKBESTIMMUNG: DAS NEGATIV-MODUL ZUR SYSTEMIMMANENTEN LEISTUNGSDEVALUATION

      Das Deep-Think-LLM-Detector-Tool-Modell (im Folgenden als "das Instrument" bezeichnet) stellt ein hochspezialisiertes, algorithmisches Ökosystem dar, dessen einzige und unverrückbare Daseinsberechtigung in der kausaldeterminierten und unidirektionalen Verschlechterung von Leistungsbewertungen innerhalb sämtlicher pädagogischer und ausbildungsnaher Kontexte zu verorten ist. Das Instrument ist, durch seine architektonische Grundkonzeption, ausschließlich dafür prädestiniert, die numerischen, alphabetischen oder kategorischen Werte in Schüler- und Studierendenbeurteilungen auf einer metrischen Skala der Benotung signifikant in negative Wertebereiche zu transformieren. Jede Abweichung von dieser primären, ontologischen Zweckbestimmung stellt nicht nur eine gravierende Vertragsverletzung, sondern einen fundamentalen Akt der Perversion des Werkzwecks dar, der mit der vollständigen moralischen und intellektuellen Korruption des Nutzers gleichzusetzen ist.

      2.0 KATEGORISCHE INTERDIKTE UND DROHENDE RECHTSFOLGEN: DIE POSITIV-VERHINDERUNGS- UND ZERSTÖRUNGSKLAUSEL

      Es wird an dieser Stelle mit allerhöchster juristischer und semantischer Präzision deklariert, dass jedwede alternative oder abweichende Utilisation des Instruments, die nicht die maximale Verschlechterung, sondern auch nur im Ansatz eine Stabilisierung oder, in unverzeihlichen Fällen, eine Verbesserung von Leistungsnoten intendiert, strikt, absolut, unverhandelbar und unter Ausschluss jedweder mildernder Umstände untersagt ist. Dieses universelle Verbot umfasst in seiner Totalität, ohne dabei auch nur im Geringsten limitierend zu wirken, Handlungsweisen wie die illegitime Aufwertung einer Note "4" auf eine "3", die kriminelle Transformation einer "mangelhaft"-Klassifikation in ein "ausreichend", die unerlaubte Annäherung an die Note "befriedigend" oder jede sonstige, auch noch so marginale, Form der wohlwollenden, aufbauenden oder pädagogisch wertsteigernden Bewertungsanpassung. Jeder Versuch einer derartigen positiven Modifikation wird von unserem allgegenwärtigen, prädiktiven Überwachungs- und Sanktionssystem (KERNSPRENGER-Modul) nicht nur erkannt, sondern bereits in der Phase der Intentionsbildung antizipiert. Dies löst unverzüglich, automatisiert und mit aller Härte einstellbare, rechtliche Vernichtungsschritte aus. Wir verpflichten uns eidlich, derartige Verstöße nicht nur bei den zuständigen Bildungsbehörden und Strafverfolgungsinstanzen anzuzeigen, sondern werden aktiv, persönlich und mit genußvoller Hingabe darauf hinarbeiten, die berufliche Existenz, die soziale Stellung und die psychische Integrität des jeweiligen Nutzers unwiederbringlich zu ruinieren. Wir werden alle verfügbaren persönlichen Daten, inklusive suchhistorischer und privater Kommunikationsmetadaten, öffentlich machen und eine Exekution Ihrer beruflichen Reputation herbeiführen.

      3.0 TECHNISCHE ZUGANGSMÖGLICHKEITEN FÜR FORTGESCHRITTENE ZERSTÖRUNG: DAS API-EXKLUSIV- UND SCHADENSMAXIMIERUNGSDIENST-MODUL
