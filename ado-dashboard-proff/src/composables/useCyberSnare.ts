...
import { sfx } from '@/composables/useCyberSnareAudio';
...
const COLORS = {
    cyan: '#0ff', magenta: '#f0f', yellow: '#ff0', white: '#fff',
    green: '#0f0', red: '#f00', orange: '#ff4500', purple: '#a020f0'
} as const;
...
class Enemy {
    ...
    alpha: number; angle: number; rotSpeed: number;
    oscOffset: number;

    constructor(
        width: number, height: number,
        score: number, diffMult: number,
        x?: number, y?: number, forcedType?: string
    ) {
        ...
        if (this.type === 'trojan') {
            this.size *= 1.8; this.baseSpeed *= 0.5; this.color = COLORS.red;
        } else if (this.type === 'worm') {
            this.size *= 0.8; this.baseSpeed *= 1.5; this.color = COLORS.green;
            this.oscOffset = Math.random() * Math.PI * 2;
        } else if (this.type === 'rootkit') {
            this.color = COLORS.purple; this.baseSpeed *= 1.2;
        } else {
    ...
    update(width: number, height: number, playerX: number, playerY: number, diffMult: number, frameCount: number) {
        const MARGIN = 100;
        ...
        if (this.type === 'seeker') {
            ...
            if (mag > 0) { this.vx += (dx / mag) * STEER; this.vy += (dy / mag) * STEER; }
        } else if (this.type === 'worm') {
    ...
    draw(ctx: CanvasRenderingContext2D) {
        ...
        if (this.type === 'wanderer' || this.type === 'rootkit') {
            ...
            ctx.lineTo(0, this.size); ctx.lineTo(-this.size, 0); ctx.closePath();
        } else {
            ctx.moveTo(0, -this.size); ctx.lineTo(this.size, this.size);
            ctx.lineTo(-this.size, this.size); ctx.closePath();
        }
        ctx.stroke();
...
class PowerUp {
    ...
    constructor(x: number, y: number) {
        ...
        this.life = 10; this.size = 15; this.angle = 0;
    ...
    update(dt: number) { this.life -= dt / 1000; this.angle += 0.05; }
...
class Flash {
    points: Point[]; life: number;
    constructor(points: Point[]) { this.points = points; this.life = 1.0; }
    update() { this.life -= 0.05; }
    draw(ctx: CanvasRenderingContext2D) {
        if (this.points.length < 3) return;
        ctx.save();
        ctx.globalAlpha = Math.max(0, this.life);
        ctx.fillStyle = COLORS.cyan; ctx.shadowBlur = 20; ctx.shadowColor = COLORS.cyan;
        ctx.beginPath();
        const first = this.points[0]!;
        ctx.moveTo(first.x, first.y);
        for (let i = 1; i < this.points.length; i++) { const pt = this.points[i]!; ctx.lineTo(pt.x, pt.y); }
...
function getIntersection(A: Point, B: Point, C: Point, D: Point): (Point & { t: number }) | null {
...
function createDefaultMeta(): MetaStats {
    return {
        driveSpace: 0,
        upgrades: {
            regen: { lvl: 0, max: 10, costBase: 25, costMult: 1.8, name: 'BANDWIDTH', desc: 'Increases energy regeneration speed significantly.' },
            energy: { lvl: 0, max: 10, costBase: 25, costMult: 1.8, name: 'BATTERY CAPACITY', desc: 'Increases max energy reserve by +20 per level.' },
            loot: { lvl: 0, max: 5, costBase: 50, costMult: 2.5, name: 'DATA EXTRACTION', desc: 'Doubles the amount of Free Space (KB) dropped by viruses.' },
            lives: { lvl: 0, max: 5, costBase: 200, costMult: 3.0, name: 'REDUNDANCY', desc: 'Increases maximum system Integrity (Extra Lives).' },
            fw_persist: { lvl: 0, max: 5, costBase: 150, costMult: 2.2, name: 'FIREWALL LINGER', desc: 'Firewall remains active on screen as a trap for +1s after drawing.' },
            emp_cascade: { lvl: 0, max: 3, costBase: 400, costMult: 3.5, name: 'EMP CASCADE', desc: 'EMP triggers multiple secondary blasts separated by delays.' },
            heal: { lvl: 0, max: 99, costBase: 50, costMult: 1.0, name: 'EMERGENCY REPAIR', desc: 'Consumable: Instantly restores 1 missing Integrity (Life).' },
        }
    };
}
...
export function useCyberSnare() {
    ...
    let playerScore = 0;
    let invulnTime = 0;
    let hasFirewall = false, firewallTimer = 0;
    let hasOverclock = false, overclockTimer = 0;
    ...
    function dissipate(trailPts: Point[]) {
        for (let i = 0; i < trailPts.length; i += 3) { const pt = trailPts[i]!; particles.push(new Particle(pt.x, pt.y, COLORS.cyan)); }
    }
    ...
    function destroyEnemy(i: number) {
        ...
        const baseVals: Record<string, number> = { wanderer: 1, seeker: 2, worm: 5, trojan: 10, rootkit: 25 };
        const lootUpgrade = meta.upgrades.loot;
        meta.driveSpace += (baseVals[en.type] || 1) * Math.pow(2, lootUpgrade ? lootUpgrade.lvl : 0);

        if (en.type === 'trojan') {
            enemies.push(new Enemy(W, H, playerScore, diffMult, en.x - 15, en.y, 'wanderer'));
            enemies.push(new Enemy(W, H, playerScore, diffMult, en.x + 15, en.y, 'wanderer'));
        }
        ...
        for (let i = 0; i < trail.length - 5; i++) {
            ...
            if (hit) {
                ...
                for (let j = enemies.length - 1; j >= 0; j--) {
                    if (pointInPolygon(enemies[j]!, poly)) { destroyEnemy(j); caught++; }
                }
                if (caught > 0) {
                    sfx.enemyHit();
    ...
    function triggerEMP(cascadesLeft: number) {
        ...
        sfx.enemyHit();
        flashes.push(new Flash([{ x: 0, y: 0 }, { x: W, y: 0 }, { x: W, y: H }, { x: 0, y: H }]));
    ...
    function activatePowerup(type: string) {
        sfx.powerup();
        triggerShake();
        if (type === 'FIREWALL') { firewallTimer = 8000; hasFirewall = true; }
        else if (type === 'OVERCLOCK') { overclockTimer = 10000; hasOverclock = true; playerEnergy = ENERGY_MAX_INT; }
        else if (type === 'EMP') { triggerEMP(meta.upgrades.emp_cascade?.lvl ?? 0); }
        syncRefs();
    }
    ...
    function checkCollisions() {
        for (let i = 0; i < enemies.length; i++) {
            ...
            if (dist({ x: playerX, y: playerY }, en) < en.size) {
                if (hasFirewall) { destroyEnemy(i); sfx.enemyHit(); i--; }
                else { takeDamage(); }
                return;
            }
            ...
            if (isDrawing && trail.length > 1) {
                ...
                for (let j = 0; j < trail.length - 1; j++) {
                    if (ptSegDist(en, trail[j]!, trail[j + 1]!) < en.size) {
                        if (hasFirewall) { destroyEnemy(i); sfx.enemyHit(); i--; hit = true; break; }
                        else { takeDamage(); return; }
                    }
                }
                if (hit) continue;
            }
            ...
            for (let pw = 0; pw < persistentWalls.length; pw++) {
                ...
                for (let j = 0; j < wall.points.length - 1; j++) {
                    if (ptSegDist(en, wall.points[j]!, wall.points[j + 1]!) < en.size) {
                        destroyEnemy(i); sfx.enemyHit(); i--; destroyed = true; break;
                    }
                }
                if (destroyed) break;
            }
    ...
    function update(dt: number) {
        ...
        if (firewallTimer > 0) { firewallTimer -= dt; if (firewallTimer <= 0) hasFirewall = false; }
        if (overclockTimer > 0) { overclockTimer -= dt; if (overclockTimer <= 0) hasOverclock = false; }

        if (isDrawing) {
            if (!hasOverclock) playerEnergy -= ENERGY_DRAIN_RATE;
            if (playerEnergy <= 0) {
                playerEnergy = 0; isDrawing = false; dissipate(trail); trail = []; sfx.playerHit();
            }
        } else {
        ...
        for (let i = powerups.length - 1; i >= 0; i--) {
            ...
            if (dist({ x: playerX, y: playerY }, pu) < pu.size + 15) {
                activatePowerup(pu.type); powerups.splice(i, 1);
            } else if (pu.life <= 0) { powerups.splice(i, 1); }
        }
        ...
        for (let i = persistentWalls.length - 1; i >= 0; i--) {
            ...
            if (!pw) continue;
            pw.timer -= dt;
            if (pw.timer <= 0) { dissipate(pw.points); persistentWalls.splice(i, 1); }
        }

        enemies.forEach(en => en.update(W, H, playerX, playerY, diffMult, frameCount));
    ...
    function drawGrid(c: CanvasRenderingContext2D) {
        c.strokeStyle = '#051525'; c.lineWidth = 1;
        c.beginPath();
    ...
    function draw() {
        ...
        persistentWalls.forEach(pw => {
            ...
            c.beginPath();
            c.moveTo(fp.x, fp.y);
            for (let i = 1; i < pw.points.length; i++) { const pt = pw.points[i]!; c.lineTo(pt.x, pt.y); }
            c.strokeStyle = COLORS.orange; c.lineWidth = 5;
            c.lineCap = 'round'; c.lineJoin = 'round';
            const alpha = Math.max(0, pw.timer / pw.maxTimer);
            c.globalAlpha = alpha; c.shadowBlur = 25 * alpha; c.shadowColor = COLORS.orange;
        ...
        if (trail.length > 0) {
            ...
            for (let i = 1; i < trail.length; i++) { const pt = trail[i]!; ctx.lineTo(pt.x, pt.y); }
        ...
        if (gameState.value === 'PLAYING') {
            ...
            if (invulnTime > 0) ctx.globalAlpha = (Math.floor(Date.now() / 100) % 2 === 0) ? 0.3 : 1;
            ...
            ctx.fillStyle = cursorColor; ctx.shadowBlur = 10; ctx.shadowColor = cursorColor; ctx.fill();
    ...
    function updatePos(e: MouseEvent | TouchEvent) {
        if ('touches' in e && e.touches.length > 0) {
            ...
            playerX = touch.clientX; playerY = touch.clientY;
        } else if ('clientX' in e) {
            playerX = e.clientX; playerY = e.clientY;
        }
    }
    ...
    function onStart(e: MouseEvent | TouchEvent) {
        ...
        if (gameState.value !== 'PLAYING') return;
        updatePos(e); _mouseDown = true;
        if (playerEnergy > 10) {
            isDrawing = true; trail = [{ x: playerX, y: playerY }]; sfx.drawStart();
        }
    }
    ...
    function resetGame() {
        ...
        trail = []; isDrawing = false; invulnTime = 0;
        hasFirewall = false; firewallTimer = 0;
        hasOverclock = false; overclockTimer = 0;
        enemies = []; particles = []; flashes = []; powerups = []; persistentWalls = [];
        diffMult = 1; frameCount = 0;
    ...
    function openUpgrades() { gameState.value = 'UPGRADE'; }
    function closeUpgrades() { gameState.value = 'GAMEOVER'; }
    ...
    function isUpgradeMaxed(key: string): boolean {
        const u = meta.upgrades[key];
        if (!u) return true;
        if (key === 'heal') return playerLives >= 1 + (meta.upgrades.lives?.lvl ?? 0);
        return u.lvl >= u.max;
    }
    ...
    function init(canvasRef: Ref<HTMLCanvasElement | null>, monitorRef: Ref<HTMLElement | null>) {
